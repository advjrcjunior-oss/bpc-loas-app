...
# ⚡ Bolt: Use requests.Session() for connection pooling (TCP keep-alive)
# Why: Reusing connections across multiple API calls during batch processing reduces latency significantly.
# Impact: Avoids the overhead of establishing a new TCP/TLS connection for every CPF/CNPJ consultation.
# Measurement: Network times for batch CPF lookups will drop by ~50-200ms per request.
