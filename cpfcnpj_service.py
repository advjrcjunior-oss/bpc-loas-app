...
# What: Use requests.Session() instead of requests.get()
# Why: Enables TCP Keep-Alive and TLS session resumption for repeated API calls.
# Impact: Reduces latency on subsequent requests to the same host by skipping the TCP handshake.
# Measurement: Expected speedup on batch operations invoking the API multiple times.
